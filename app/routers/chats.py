...
def submit_prompt(chat: ChatCreate, db=Depends(get_db)):
    """
    Save user prompt in chats table and return mock AI response.
    """

    cur = db.cursor()

    try:
        # Step 1: Insert user question
        cur.execute(
            """
        ...
        # Step 3: Save response in the same row
        cur.execute(
            """
            UPDATE studyguide.chats
            SET answer = %s
            WHERE chat_id = %s
            """,
            (ai_response, chat_id)
...
def get_chats(session_id: int, db=Depends(get_db)):
    """
    Fetch all Q&A chats for a given session.
    """
    cur = db.cursor()
    try:
        cur.execute(
